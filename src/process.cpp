...
struct Job {
	...
	Job(RangePartitioning& range_partitioning,
		const std::vector<std::wstring>& paths,
		std::vector<Duplicate>& duplicates_visual,
		std::vector<Duplicate>& duplicates_time,
		std::vector<Duplicate>& duplicates_location,
...
static void thread_worker(Job* const job) {
	...
	for (;;) {
		auto range = job->range_partitioning.get_next();

		if (range == nullptr || job->force_thread_exit || !ErrorTrap::is_good())
			break;

		while (const auto index_pair = range->get_next()) {
			if (job->images[index_pair->first] == nullptr)
				job->images[index_pair->first] =
					std::make_shared<Image>(job->paths[index_pair->first]);
			if (job->images[index_pair->second] == nullptr)
				job->images[index_pair->second] =
					std::make_shared<Image>(job->paths[index_pair->second]);

			const auto image_1 = job->images[index_pair->first];
			const auto image_2 = job->images[index_pair->second];

			auto s1 = image_1->get_image_size();
			auto ar1 = static_cast<float>(s1.width) / s1.height;
			ar1 = std::max(ar1, 1/ar1);

			auto s2 = image_2->get_image_size();
			auto ar2 = static_cast<float>(s2.width) / s2.height;
...
std::vector<std::vector<Duplicate>> process(Window& window, const std::vector<std::wstring>& paths) {
	// prepare job
	RangePartitioning range_partitioning(paths.size());
	...
	while (job.range_partitioning.get_progress() < job.range_partitioning.get_size()) {
		if (window.get_event().type == Event::Type::quit) {
			job.force_thread_exit = true;
			break;
		}
		window.set_progressbar_progress(0, job.range_partitioning.get_progress(), job.range_partitioning.get_size());
	}

	// set progressbar to 100 % (cosmetic)
